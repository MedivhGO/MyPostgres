...
postgresGetForeignRelSize(PlannerInfo *root,
						  ...
						  Oid foreigntableid)
{
	PgFdwRelationInfo *fpinfo; // 将用来存储fdw生成路径,路径选择,最终路径结果的所有信息.
	ListCell   *lc;
	...
	fpinfo->table = GetForeignTable(foreigntableid); // 查找 外部表的 catalog info
	fpinfo->server = GetForeignServer(fpinfo->table->serverid); // 获得外部表的 server info
	// 用这些接口去实现 fdwapi中的函数
	 ...
	 */
	fpinfo->use_remote_estimate = false; // 是否对远端数据源进行代价评估
	 ...
	 */
	// 在下面的 classifyConditions() 中，
	// 对输入的 SQL 语句中的 where 限制条件进行逐项归类，
	// 将之分为 remote_conds 和 local_conds，前者可以发送到 remote 端处理，后者不能。
	 ...
	 */
	fpinfo->attrs_used = NULL; //这片代码确认需要向远端数据源请求的列名
	...
	cost_qual_eval(&fpinfo->local_conds_cost, fpinfo->local_conds, root); // 本地代价评估
	 ...
	 */
	if (fpinfo->use_remote_estimate) // 能估计外部表时构造explain语句并使用返回估计的值
	{
		/*
					   ...
					   Plan *outer_plan) //此函数用于生成访问目标外部表的ForeignScan计划节点
{
	 ...
	 */
	initStringInfo(&sql); // 生成用于查询外部数据源的sql
						...
						Cost *p_startup_cost, Cost *p_total_cost)
{
	...
	if (fpinfo->use_remote_estimate)
	{
		 ...
		 */
		initStringInfo(&sql);
		appendStringInfoString(&sql, "EXPLAIN "); // 需要对远端数据源进行评估，构造sql语句前添加explain
		// 并获取explain的结果。
		deparseSelectStmtForRel(&sql, root, foreignrel, fdw_scan_tlist,
