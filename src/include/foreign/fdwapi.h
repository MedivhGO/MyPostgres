...
typedef TupleTableSlot *(*IterateForeignScan_function) (ForeignScanState *node); // 5

typedef bool (*RecheckForeignScan_function) (ForeignScanState *node,
											 TupleTableSlot *slot);

typedef void (*ReScanForeignScan_function) (ForeignScanState *node); // 6

typedef void (*EndForeignScan_function) (ForeignScanState *node); // 7
 ...
 */
typedef struct FdwRoutine // 这个结构体描述了一个外部数据包装器程序
{ // 其主要内容是函数指针实现的回调函数
// 扫描相关的函数 是必须要实现的
	NodeTag		type;
