...
#define IS_OTHER_REL(rel) \
	((rel)->reloptkind == RELOPT_OTHER_MEMBER_REL || \
	 (rel)->reloptkind == RELOPT_OTHER_JOINREL || \
	 (rel)->reloptkind == RELOPT_OTHER_UPPER_REL)
// 路径生成和选择涉及的所有操作几乎都是针对这个结构进行的.
typedef struct RelOptInfo //对于任意给定的一组baserel的集合,只有一个RelOptInfo结构.
{//生成路径的最终结果始终存放在其中,生成和选择路径所需的许多数据也存放在其中.
	NodeTag		type; //节点类型

	RelOptKind	reloptkind;  //关系的类型(基本关系,连接关系,其他成员关系)

	/* all relations included in this RelOptInfo */
	Relids		relids;			/* set of base relids (rangetable indexes) */ //关系所涉及的基本表(在范围表中的编号)
	...
	/* materialization information */ // 多个基本关系生成该关系的路径(多条可行路径)
	List	   *pathlist;		/* Path structures *///生成执行计划就是从下往上处理这个属性的过程,选择有成本较优的节点,删除无用的节点最后得到执行计划
	List	   *ppilist;		/* ParamPathInfos used in pathlist */
	List	   *partial_pathlist;	/* partial Paths */
	struct Path *cheapest_startup_path; // 最小的启动代价路径
	struct Path *cheapest_total_path; // 最小的总代价路径
	struct Path *cheapest_unique_path; // 最小的唯一代价路径
	...
	RTEKind		rtekind;		/* RELATION, SUBQUERY, FUNCTION, etc */ // 基本关系的类型(普通关系,子查询等)
	AttrNumber	min_attr;		/* smallest attrno of rel (often <0) */ //关系中最小的字段编号
	AttrNumber	max_attr;		/* largest attrno of rel */ //关系中最大的字段编号
	Relids	   *attr_needed;	/* array indexed [min_attr .. max_attr] */ //连接关系中哪些字段是需要的
	int32	   *attr_widths;	/* array indexed [min_attr .. max_attr] */ //每个字段的宽度的估计
	List	   *lateral_vars;	/* LATERAL Vars and PHVs referenced by rel */
	Relids		lateral_referencers;	/* rels that reference me laterally */
	List	   *indexlist;		/* list of IndexOptInfo *///如RelOptInfo中的indexlist用于表示这张表的索引信息，用于判断是否可以用上索引。
	List	   *statlist;		/* list of StatisticExtInfo */
	BlockNumber pages;			/* size estimates derived from pg_class */ //关系占用的页面数
	double		tuples; //关系的元组数
	double		allvisfrac;
	Bitmapset  *eclass_indexes; /* Indexes in PlannerInfo's eq_classes list of
								 * ECs that mention this rel */
	PlannerInfo *subroot;		/* if subquery */ //子查询的计划
	List	   *subplan_params; /* if subquery */ //子查询的参数
	...
	List	   *baserestrictinfo;	/* RestrictInfo structures (if base rel) */ //基本关系上的约束信息
	QualCost	baserestrictcost;	/* cost of evaluating the above */ //对上述约束信息的代价估计
	Index		baserestrict_min_security;	/* min security_level found in
											 * baserestrictinfo */
	List	   *joininfo;		/* RestrictInfo structures for join clauses 连接子句中调用该关系时的约束信息.
								 * involving this rel */
	bool		has_eclass_joins;	/* T means joininfo is incomplete */ //是否存在等值连接
 ...
 */
typedef struct Path //路径是一个树结构的表示
{
	NodeTag		type;  // 节点类型

	NodeTag		pathtype;		/* tag identifying scan/join method */ //标记扫描/连接方法

	RelOptInfo *parent;			/* the relation this path can build */ // 此路径所构建的关系.
	...
	double		rows;			/* estimated number of result tuples */
	Cost		startup_cost;	/* cost expended before fetching any tuples */ //启动代价估计
	Cost		total_cost;		/* total cost (assuming all tuples fetched) */ //总代价估计
